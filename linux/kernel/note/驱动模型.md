...
struct device_driver {
	...
	void (*sync_state)(struct device *dev);		// 在所有与该设备关联的状态跟踪使用者（在late_initcall时存在）成功绑定到驱动程序后，调用此函数将设备状态同步到软件状态。如果设备没有使用者，此函数将在late_initcall_sync级别被调用。如果设备的使用者从未绑定到驱动程序，则在它们绑定之前，此函数永远不会被调用。
