...
Y-Cable: 通信 / 网络硬件术语, Y 型线缆 (一台服务器接两个交换机实现主备切换), 相关供应商为 broadcom / credo

**核心功能**：SONiC Y型线缆线接口与更新守护服务, 使用gRPC协议实现一台服务器接两个交换机实现主备切换
...
- /etc/sonic/grpc_secrets.json  (RPC秘钥配置)
  - GRPCCLIENT
    ...
    - certs:
      ...
      - ca_crt: ''
      - grpc_ssl_credential: ''
- src/sonic-platform-common/sonic_y_cable/y_cable.py (目录下存在支持的YCable适配器, 如 broadcom/credo/microsoft)
...
**重要文件**：
- /usr/share/sonic/platform/sensors.conf
- /usr/share/sonic/platform/fancontrol
